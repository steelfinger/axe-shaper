...
| Neck pocket | 40 × 98.425mm | `TEMPLATE_NECK_POCKET_SPEC.r_bass_style` — a body-specific override of `GENERIC_POCKET_SPEC.bass.glued` (63.5 × 98.425mm), which W2 recorded as a known gap because it repeats the bolt-on bass pocket. A neck-through R-Style has no bass-heel mortise; what the pocket rout represents here is the centre strip, and 40mm is what this body's own contour is built to — its `s_pocket_left`/`s_pocket_right` anchors sit at X=±20 | Medium (notional, matched to this file's own contour) |
