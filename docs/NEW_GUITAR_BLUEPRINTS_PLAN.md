...
`main` at fc256b3. All four blueprints are built and committed on
`feature/new-guitar-blueprints` (draft PR #7); what remains is listed per blueprint.
...
- **Status (2026-09-24): built, committed on `feature/new-guitar-blueprints`.** `gibson_explorer.axe.svg` is
...
- **Still open:** `npm run fixtures:check` fails on "missing:" all four new
  blueprints (`gibson_explorer`, `prs_style`, `semi_hollow_double_cut`,
  `semi_hollow_single_cut`) until the iOS repo writes its own copies into
...
All four blueprints are implemented; the remaining work is the iOS fixtures, the
viewer's `frontRoutes` rendering, and the owner's review of each outline.
