...
async function main() {
  ...
  try {
    ...
    // for single_cut and prs_style, the two with a bodyTop. Accepting any supported version
    ...
    const bundledBlueprints = readdirSync(BLUEPRINT_DIR)
      ...
      .sort();
    invariant(bundledBlueprints.length === 18, `expected 18 bundled blueprints, found ${bundledBlueprints.length}`);
